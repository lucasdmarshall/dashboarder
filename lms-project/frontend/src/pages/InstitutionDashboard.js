...
const InstitutionDashboard = () => {
  ...
  const [selectedInstructorForAssignment, setSelectedInstructorForAssignment] = useState(null);
  ...
  const handleTabChange = (index) => {
    setTabIndex(index);
    if (index !== 2 && index !== 4) { // If not on the Add Student or Add Instructor tab
  ...
  // Handle input change for the new instructor form
  ...
  const handleAddInstructor = () => {
    ...
    if (!newInstructor.name || !newInstructor.id || !newInstructor.email || !newInstructor.subject) {
      toast({
        title: "Missing Information",
    ...
    if (instructors.some(instructor => instructor.id === newInstructor.id)) {
      toast({
        title: "Duplicate ID",
        description: "An instructor with this ID already exists.",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
    ...
    toast({
      ...
      description: `${newInstructor.name} has been added to the Instructor List.`,
      status: "success",
      duration: 3000,
      isClosable: true,
    });

    // Switch to the Instructor List tab
    setTabIndex(3);
  };
  ...
  const handleDeleteInstructor = (id) => {
    ...
    const instructorToDelete = instructors.find(instructor => instructor.id === id);
    ...
    toast({
      ...
      description: `${instructorToDelete.name} has been removed from the Instructor List.`,
      status: "success",
      duration: 3000,
      isClosable: true,
    });
  ...
  const filteredInstructors = instructors.filter(instructor => {
    const searchTermLower = instructorSearchTerm.toLowerCase();
    return (
      instructor.name.toLowerCase().includes(searchTermLower) ||
      instructor.id.toLowerCase().includes(searchTermLower) ||
      instructor.email.toLowerCase().includes(searchTermLower) ||
      instructor.subject.toLowerCase().includes(searchTermLower)
    );
  });

  const [classForAssignment, setClassForAssignment] = useState(null);
  const [isClassAssignmentModalOpen, setIsClassAssignmentModalOpen] = useState(false);

  // Handle instructor selection for class assignment
  const handleSelectInstructorForAssignment = (instructor) => {
    setSelectedInstructorForAssignment(instructor);
    setIsClassAssignmentModalOpen(true);
  };
  
  // Handle assigning instructor to class
  const handleAssignInstructorToClass = () => {
    if (!selectedInstructorForAssignment || !classForAssignment) {
      toast({
        title: "Missing Information",
        description: "Please select both an instructor and a class for assignment.",
        status: "warning",
        duration: 3000,
        isClosable: true,
      });
    ...
    toast({
      ...
      description: `${selectedInstructorForAssignment.name} has been assigned to ${classForAssignment.name}.`,
      status: "success",
      duration: 3000,
      isClosable: true,
    });
    ...
    // Close the modal
    setIsClassAssignmentModalOpen(false);
    setSelectedInstructorForAssignment(null);
    setClassForAssignment(null);
  };
  
  // Get student count for a class
  const getClassStudentCount = (classItem) => {
    if (!classItem || !classItem.courses) return 0;
    
    // Calculate unique students across all courses in the class
  ...
  return (
    <Box>
      ...
      >
        ...
        <Tabs isFitted variant="enclosed" colorScheme="red" index={tabIndex} onChange={handleTabChange}>
          <TabList mb="1em">
            ...
            <Tab _selected={{ color: "white", bg: "#640101" }} fontWeight="medium">Instructor List</Tab>
            <Tab _selected={{ color: "white", bg: "#640101" }} fontWeight="medium">Add Instructor</Tab>
          </TabList>
          ...
          <TabPanels>
            ...
            <TabPanel>
              <Box mb={5}>
                <InputGroup>
                  ...
                  <Input 
                    placeholder="Search instructors by name, ID, or subject..." 
                    borderColor="gray.300"
                    _focus={{ borderColor: "#640101", boxShadow: "0 0 0 1px #640101" }}
                    ...
                    {filteredInstructors.length > 0 ? (
                      filteredInstructors.map((instructor) => (
                        <Tr key={instructor.id}>
                          ...
                          <Td>
                            <Tooltip label={instructor.email} placement="top" hasArrow>
                              <Text noOfLines={1} maxW="180px">{instructor.email}</Text>
                            </Tooltip>
                          </Td>
                              ...
                              <IconButton
                                ...
                                onClick={() => {
                                  toast({
                                    ...
                                    description: `Viewing details for ${instructor.name}`,
                                    status: "info",
                                    duration: 2000,
                                    isClosable: true
                                  });
                              ...
                              <IconButton
                                ...
                                onClick={() => {
                                  toast({
                                    ...
                                    description: `Editing details for ${instructor.name}`,
                                    status: "info",
                                    duration: 2000,
                                    isClosable: true
                                  });
            ...
            <TabPanel>
              <VStack spacing={5} align="stretch">
                <Tabs variant="enclosed-colored" colorScheme="red" size="md">
                  <TabList>
                    <Tab _selected={{ color: "white", bg: "#640101" }}>Add New Instructor</Tab>
                    <Tab _selected={{ color: "white", bg: "#640101" }}>Assign Instructor to Class</Tab>
                  </TabList>
                  
                  <TabPanels>
                    {/* Add New Instructor SubTab */}
                    <TabPanel>
                      <Text fontSize="lg" fontWeight="bold" color="#640101">Add New Instructor</Text>
                    ...
                    {/* Assign Instructor SubTab */}
                    <TabPanel>
                      <Text fontSize="lg" fontWeight="bold" color="#640101" mb={4}>Assign Instructor to Class</Text>
                      ...
                      <Box overflowX="hidden" borderWidth="1px" borderRadius="lg" borderColor="gray.200" mb={6}>
                                ...
                                <Tr key={instructor.id}>
                                  ...
                                  <Td>
                                    <Button
                                      ...
                                      onClick={() => handleSelectInstructorForAssignment(instructor)}
                                    >
                                      Assign
                                    </Button>
                                  </Td>
      ...
      {/* Instructor Class Assignment Modal */}
      <Modal isOpen={isClassAssignmentModalOpen} onClose={() => setIsClassAssignmentModalOpen(false)} size="lg">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader bg="#640101" color="white" borderTopRadius="md">
          ...
          <ModalBody py={4}>
            {selectedInstructorForAssignment && (
              <Box mb={4} p={3} borderWidth="1px" borderRadius="md" bg="blue.50">
                <Heading size="sm">Selected Instructor:</Heading>
                <Text>{selectedInstructorForAssignment.name} ({selectedInstructorForAssignment.id})</Text>
                <Text fontSize="sm">Subject: {selectedInstructorForAssignment.subject}</Text>
              </Box>
            )}
            
            <Text fontWeight="bold" mt={4} mb={2}>Select Class for Assignment:</Text>
            
            <SimpleGrid columns={1} spacing={3}>
              {grades.map(grade => (
                <Box key={grade.id}>
                  <Heading size="sm" mb={2} color="#640101">{grade.name}</Heading>
                  <SimpleGrid columns={2} spacing={3}>
                    {grade.classes.map(classItem => (
                      <Box 
                        key={classItem.id}
                        onClick={() => setClassForAssignment(classItem)}
                        cursor="pointer"
                        borderWidth="1px"
                        borderRadius="lg"
                        p={3}
                        bg={classForAssignment?.id === classItem.id ? "rgba(100, 1, 1, 0.1)" : "white"}
                        _hover={{ borderColor: "#640101" }}
                      >
                        <Flex justify="space-between" align="center">
                          <Text fontWeight="medium">{classItem.name}</Text>
                          <Badge colorScheme="blue">{getClassStudentCount(classItem)} students</Badge>
                        </Flex>
          ...
          <ModalFooter>
            <Button variant="ghost" mr={3} onClick={() => setIsClassAssignmentModalOpen(false)}>
              Cancel
            </Button>
            <Button 
              colorScheme="red" 
              bg="#640101"
              onClick={handleAssignInstructorToClass}
              isDisabled={!selectedInstructorForAssignment || !classForAssignment}
            >
              Assign Instructor
            </Button>
          </ModalFooter>
